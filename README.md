...
| `/w` `/t` `/s` | search the front app’s windows / tabs / every pressable element (`/w`·`/t` also list results below the panel) |
...
| `⌘<` / `⌘>` | move to the previous / next display (opens on the cursor's display) |
| `⌘R` | rescan hints |
