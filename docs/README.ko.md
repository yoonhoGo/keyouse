...
실행하면 메뉴바에 아이콘이 생기고, 터미널에서 띄워도 프로세스가 분리되어(detach) 프롬프트가 바로 돌아온다. 다시 실행하면 기존 인스턴스를 종료하고 새 인스턴스로 교체된다. 종료는 메뉴바 › 종료 또는 `pkill -f keyouse`.
...
| `⌘L` | 링크만 표시 (토글) |
| `⌃I` | 첫 입력 필드로 포커스 |
| `/w` `/t` `/s` | 열린 창(모든 앱) / 앱의 탭 / 모든 클릭 가능 요소 검색 (`/w`·`/t`는 패널 아래 결과 리스트도 표시) |
| `>` *(또는 `⌘P`)* | 명령 팔레트 — 앞 앱의 메뉴 명령 검색·실행 (단축키 표기, `⏎`/`숫자`로 실행) |
| `⌘?` | 단축키 가이드 표시/숨김 |
| `⌘Q` | 앞 앱 종료 |
| `⌘W` / `⌘⇧W` | 현재 탭 / 창 닫기 |
| `⌘Tab` | 창 피커 열기 · 다음 창 (`⇧⌘Tab` 이전, `⌘←→↑↓` 이동, `⌘` 떼면 선택) |
| `⌘<` / `⌘>` | 이전 / 다음 디스플레이로 이동 (기본은 포커스된 윈도우가 있는 디스플레이에서 열림) |
| `⌘R` | 힌트 다시 스캔 |
